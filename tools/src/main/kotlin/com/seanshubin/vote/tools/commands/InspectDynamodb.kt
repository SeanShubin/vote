...
private const val PROD_HELP = "Target real AWS DynamoDB instead of DynamoDB Local."

class InspectDynamodbTables : CliktCommand(name = "inspect-dynamodb-tables") {
    private val prod by option("--prod", help = PROD_HELP).flag()
    override fun help(context: Context) = "Show DynamoDB table overview."
    override fun run() = inspectDynamodbTables(prod)
}
...
class InspectDynamodbUsers : CliktCommand(name = "inspect-dynamodb-users") {
    ...
    override fun run() = inspectDynamodbUsers(prod)
}
...
class InspectDynamodbElections : CliktCommand(name = "inspect-dynamodb-elections") {
    ...
    override fun run() = inspectDynamodbElections(prod)
}

class InspectDynamodbCandidates : CliktCommand(name = "inspect-dynamodb-candidates") {
    private val election: String? by argument(name = "ELECTION").optional()
    private val prod by option("--prod", help = PROD_HELP).flag()
    override fun help(context: Context) = "List candidates (optionally filtered by election)."
    override fun run() = inspectDynamodbCandidates(prod, election)
}
...
class InspectDynamodbVoters : CliktCommand(name = "inspect-dynamodb-voters") {
    ...
    override fun run() = inspectDynamodbVoters(prod, election)
}
...
class InspectDynamodbBallots : CliktCommand(name = "inspect-dynamodb-ballots") {
    ...
    override fun run() = inspectDynamodbBallots(prod, election)
}
...
class InspectDynamodbEventLog : CliktCommand(name = "inspect-dynamodb-event-log") {
    ...
    override fun run() = inspectDynamodbEventLog(prod)
}
...
class InspectDynamodbSyncState : CliktCommand(name = "inspect-dynamodb-sync-state") {
    ...
    override fun run() = inspectDynamodbSyncState(prod)
}

class InspectDynamodbRaw : CliktCommand(name = "inspect-dynamodb-raw") {
    private val tableName: String? by argument(name = "TABLE").optional()
    private val prod by option("--prod", help = PROD_HELP).flag()
    override fun help(context: Context) = "Raw scan of a DynamoDB table."
    override fun run() = inspectDynamodbRaw(prod, tableName)
}
...
class InspectDynamodbRawAll : CliktCommand(name = "inspect-dynamodb-raw-all") {
    ...
    override fun run() = inspectDynamodbRawAll(prod)
}
...
class InspectDynamodbRawKeys : CliktCommand(name = "inspect-dynamodb-raw-keys") {
    ...
    override fun run() = inspectDynamodbRawKeys(prod)
}
...
class InspectDynamodbAll : CliktCommand(name = "inspect-dynamodb-all") {
    ...
    override fun run() {
        ...
        println("Target: ${DynamoClient.describe(prod)}")
        println("=========================================")
        println()
        inspectDynamodbTables(prod)
        println()
        inspectDynamodbUsers(prod)
        println()
        inspectDynamodbElections(prod)
        println()
        inspectDynamodbBallots(prod, null)
    }
}

// --- inspection logic (callable both from clikt commands and from inspect-dynamodb-all) ---

private fun targetHeader(prod: Boolean): String = "Target: ${DynamoClient.describe(prod)}"

private fun inspectDynamodbTables(prod: Boolean) = runBlocking {
    println("=== DynamoDB Tables Overview (Single-Table Design) ===")
    println(targetHeader(prod))
    println()
...
private fun inspectDynamodbUsers(prod: Boolean) = runBlocking {
...
private fun inspectDynamodbElections(prod: Boolean) = runBlocking {
...
private fun inspectDynamodbCandidates(prod: Boolean, election: String?) = runBlocking {
...
private fun inspectDynamodbVoters(prod: Boolean, election: String?) = runBlocking {
...
private fun inspectDynamodbBallots(prod: Boolean, election: String?) = runBlocking {
...
private fun inspectDynamodbEventLog(prod: Boolean) = runBlocking {
...
private fun inspectDynamodbSyncState(prod: Boolean) = runBlocking {
...
private fun inspectDynamodbRaw(prod: Boolean, tableName: String?) = runBlocking {
    val table = tableName ?: run {
        println("Usage: scripts/dev inspect-dynamodb-raw <table-name> [--prod]")
        println()
...
private fun inspectDynamodbRawAll(prod: Boolean) = runBlocking {
    println("=========================================")
...
private fun inspectDynamodbRawKeys(prod: Boolean) = runBlocking {
