...
Here's the idea: imagine the candidates as a round-robin tournament. Take any two of them — say, Kotlin vs. Rust — and ask every voter, "On your ballot, did you rank Kotlin higher than Rust, or Rust higher than
Kotlin?" Whichever name got the higher rank from more voters wins that matchup. Repeat for every possible pair — Kotlin vs. Go, Rust vs. Go, and so on. Now you have a complete record of who beats whom,
one-on-one.
...
- Sometimes nobody beats everyone directly. Maybe A beats B, B beats C, but C beats A — like rock-paper-scissors. In that case the system looks at the "strongest paths" between candidates: if lots of voters
  prefer A to B, and lots prefer B to C, that's evidence A is also stronger than C, even though they didn't face off cleanly. The standings come out of weighing all those chains.
...
The upshot: nobody is picked just because they had a passionate minority. The winner has to hold up against every other candidate, one matchup at a time.
