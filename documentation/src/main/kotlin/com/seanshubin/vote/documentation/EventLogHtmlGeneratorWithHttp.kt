...
class EventLogHtmlGeneratorWithHttp(private val recorder: DocumentationRecorder) {
    ...
    private fun formatEvent(event: DomainEvent): String = when (event) {
        ...
        is DomainEvent.ElectionNameChanged ->
            "Election <strong>${event.oldName}</strong> renamed to <strong>${event.newName}</strong>"
        ...
        is DomainEvent.ElectionManagerAdded ->
            "<strong>${event.userName}</strong> added as manager of <strong>${event.electionName}</strong>"
        is DomainEvent.ElectionManagerRemoved ->
            "<strong>${event.userName}</strong> removed as manager of <strong>${event.electionName}</strong>"
